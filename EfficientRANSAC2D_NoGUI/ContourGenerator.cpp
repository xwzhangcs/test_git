	...
	for (int j = 0; j < shapes.size(); j++) {
		if (efficient_ransac::Circle* circle = dynamic_cast<efficient_ransac::Circle*>(shapes[j].second.get())) {
			...
			if (efficient_ransac::Circle* circle2 = dynamic_cast<efficient_ransac::Circle*>(shapes[j2].second.get())) {
				...
				cv::Point2f int_pt;
				double tab, tcd;
				if (util::lineLineIntersection(shapes[j].second->start_point, shapes[j].second->end_point, shapes[j2].second->end_point, shapes[j2].second->start_point, &tab, &tcd, false, int_pt)) {
					bool valid = false;
