...
namespace efficient_ransac {
	...
	std::vector<std::pair<int, std::shared_ptr<PrimitiveShape>>> EfficientRANSAC::detectLines(const std::vector<cv::Point2f>& polygon, int line_num_iterations, int line_min_points, float line_max_error, float line_cluster_epsilon, float line_min_length, float line_angle_threshold, std::vector<float>& principal_orientations) {
		std::vector<Point> points(polygon.size());
		for (int i = 0; i < polygon.size(); i++) {
			points[i] = Point(polygon[i].x, polygon[i].y);
		}
